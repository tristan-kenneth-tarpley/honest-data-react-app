...
export const ChartItemEditing: React.FC<{
    ...
    const [date, renderDate] = useState<{from?: DayRange['from'], to?: DayRange['to']}>({
        from: props.from,
        to: props.to
    })

    const onResetDate = () => {
        if (props.uid) props.setChartDateRange({
            from: undefined,
            to: undefined
        }, props.uid)
        ...
        <div className={classNames({
            ...
            { props.dataViewType === viewTypes.timeSeries && (
                <React.Fragment>
                    <Text size="sm" len="long">
                        Date range:
                        <span onClick={()=> onResetDate()} className="reset">reset to default </span>
                    </Text>
                    <DateRange
            ...
            <div className="sidebar__item-edit-chartSelector">
                <ChartSelection
                    ...
                    setActiveChartType={props.adding ? localSetActiveChartType : props.setActiveChartType}
                    activeChartType={props.adding ? localActiveChartType : props.activeChartType!}
                />
            </div>
            <br />
